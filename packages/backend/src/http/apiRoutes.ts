...
const SCAN_LIMIT_DEFAULT = 50;
const SCAN_LIMIT_MAX = 500;
...
export const apiRoutes: FastifyPluginAsync<{ deps: ApiDeps }> = (app, opts) => {
  ...
  app.get('/scans', async (request) => {
    ...
    const parsed = raw !== undefined ? Number.parseInt(raw, 10) : SCAN_LIMIT_DEFAULT;
    const limit = Number.isFinite(parsed)
      ? Math.min(Math.max(parsed, 1), SCAN_LIMIT_MAX)
      : SCAN_LIMIT_DEFAULT;
    const response: ScansResponse = {
      count: deps.scanLog.count(),
      scans: deps.scanLog.recent(limit),
    };
    return response;
  });
