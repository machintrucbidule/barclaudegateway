...
- **Tags** (published from a `vX.Y.Z` git tag): the exact `X.Y.Z` (immutable, reproducible), the
  ...
  - Pin `BCG_TAG=0.2.2` for a fixed, reproducible deployment (current latest stable).
...
2. In the stack's **Environment variables**, set:
   ...
   - `BCG_TAG` — `latest` (auto-update) or a pinned version like `0.2.2` (optional).
...
```sh
# bump the version in the root package.json, commit, then (example for 0.2.2):
git tag v0.2.2
